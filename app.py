...
# Função com CACHE para leitura de dados
@st.cache_data(ttl=300)
def carregar_dados():
    try:
        df_o = conn.read(worksheet="Obras")
    except Exception:
        df_o = pd.DataFrame(columns=["nome_ptd", "data_corte", "dp_aplicavel", "data_descargas_parciais", "arquivado", "motivo_arquivo"])
    ...
    if not df_o.empty:
        if "arquivado" not in df_o.columns:
            df_o["arquivado"] = "Não"
        if "motivo_arquivo" not in df_o.columns:
            df_o["motivo_arquivo"] = ""
...
# Painel da Sidebar: Arquivo de Obras (Desarquivamento Corrigido)
with st.sidebar.expander("📦 Obras em Arquivo", expanded=True):
    obras_arquivadas = []
    hoje = datetime.now().date()
    
    for idx, r in df_obras.iterrows():
        dt_fim = extrair_ultima_data(r["data_corte"])
        is_arq = r.get("arquivado") == "Sim"
        motivo = r.get("motivo_arquivo", "")
        passou_data = (dt_fim != date.min and dt_fim < hoje)
        
        # Só considera arquivado por data se NÃO tiver sido explicitamente desarquivado (motivo == "manual")
        if is_arq or (passou_data and motivo != "manual"):
    ...
    if not obras_arquivadas:
        ...
        for arq_ptd in obras_arquivadas:
            ...
            if c_a2.button("Desarquivar", key=f"unarch_{arq_ptd}"):
                ...
                if len(idx_found) > 0:
                    # Altera o estado e marca motivo como "manual" para bloquear o arquivamento automático por data
                    df_obras.loc[idx_found[0], "arquivado"] = "Não"
                    df_obras.loc[idx_found[0], "motivo_arquivo"] = "manual"
...
if st.session_state.ptd_selecionado is None:
    ...
    if df_obras.empty or "nome_ptd" not in df_obras.columns:
        ...
        for idx, row in df_obras_ordenadas.iterrows():
            ...
            motivo = str(row.get("motivo_arquivo", ""))
            dt_fim = extrair_ultima_data(dt_corte_val)
            
            passou_data = (dt_fim != date.min and dt_fim < hoje)

            # Lógica de Ocultação: Esconde se for arquivado explicitamente OU se a data passou (desde que não tenha sido desarquivado manualmente)
            if is_arquivado or (passou_data and motivo != "manual"):
                continue
            ...
            with st.container(border=True):
                ...
                if col5.button("📦 Arquivar", key=f"arch_btn_{ptd_key}", use_container_width=True):
                    ...
                    df_obras.loc[idx, "motivo_arquivo"] = "user"
    ...
    if len(obra_row_idx) > 0:
        ...
        if modificado:
            if deve_arquivar:
                ...
                df_obras.loc[idx_obra, "motivo_arquivo"] = "cancelado"
